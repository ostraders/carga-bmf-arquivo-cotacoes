...
sudo docker ps --format "{{.Names}}" | grep 'ostraders' &> /dev/null

if [ $? == 0 ]; then
   echo "Containers já existem no sistema operacional"
   sudo chmod 777 /var/run/docker.sock
   sudo docker start --detach-keys --interactive carga-bmf-arquivo-cotacoes_kafka_1
   sudo docker start --detach-keys --interactive carga-bmf-arquivo-cotacoes_pg-dbbmf_1
   sudo docker start --detach-keys --interactive carga-bmf-arquivo-cotacoes_pg-calcula-dbbmf_1
else
