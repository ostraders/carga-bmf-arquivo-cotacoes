...
@Slf4j
@Service
public class GerenciadorArquivosServiceImpl implements GerenciadorArquivosService, Serializable {
  ...
  public File renomearArquivo(File arquivo) {
    arquivo.renameTo(new File(CAMINHO_ARQUIVO_ENTRADA.getCaminho(),
        arquivo.getName() + LOCK));
    arquivo = new File(CAMINHO_ARQUIVO_ENTRADA.getCaminho(),
        arquivo.getName() + LOCK);
    return arquivo;
  }
