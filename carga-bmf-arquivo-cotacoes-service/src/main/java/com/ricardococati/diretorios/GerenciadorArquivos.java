...
public class GerenciadorArquivos implements IGerenciadorArquivos, Serializable{
	...
	public File renomearArquivo(File arquivo) {
		arquivo.renameTo(new File(CaminhoArquivoEnum.CAMINHO_ARQUIVO_ENTRADA.getCaminho(), arquivo.getName()+ LOCK));
		arquivo = new File(CaminhoArquivoEnum.CAMINHO_ARQUIVO_ENTRADA.getCaminho(), arquivo.getName()+ LOCK);
		return arquivo;
	}
